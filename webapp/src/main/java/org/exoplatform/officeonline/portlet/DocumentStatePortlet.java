...
public class DocumentStatePortlet extends GenericPortlet {
  ...
  protected void doView(final RenderRequest request, final RenderResponse response) {
    ...
    if (LOG.isDebugEnabled()) {
      ...
      // OfficeonlineDocumentsLifecycle
      String userId = (String) context.getAttribute(OfficeOnlineContext.USERID_ATTRIBUTE);
      if (userId != null) {
        String nodeWs = (String) context.getAttribute(OfficeOnlineContext.DOCUMENT_WORKSPACE_ATTRIBUTE);
        String nodePath = (String) context.getAttribute(OfficeOnlineContext.DOCUMENT_PATH_ATTRIBUTE);

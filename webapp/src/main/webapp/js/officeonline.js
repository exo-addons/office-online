...
(function($, cCometD, redux) {
  ...
  var configGet = function(workspace, path) {
    var request = $.ajax({
      type : "GET",
      url : prefixUrl + "/portal/rest/officeonline/editor/config/" + workspace + path,
      dataType : "json"
    });
  ...
  var configGetByKey = function(key) {
    var request = $.ajax({
      type : "GET",
      url : prefixUrl + "/portal/rest/officeonline/editor/config/" + key,
      dataType : "json"
    });
    return initRequest(request);
  };
  ...
  var documentPost = function(workspace, path) {
    var request = $.ajax({
      type : "POST",
      url : prefixUrl + "/portal/rest/officeonline/editor/document/" + workspace + path,
      dataType : "json"
    });
    return initRequest(request);
  };
  var stateGet = function(userId, fileKey) {
    var request = $.ajax({
      type : "GET",
      url : prefixUrl + "/portal/rest/officeonline/editor/state/" + userId + "/" + fileKey,
      dataType : "json"
    });
    return initRequest(request);
  };
  */
  ...
  function Editor() {
    ...
    var subscribeDocument = function(docId) {
      ...
      var subscription = cometd.subscribe("/eXo/Application/Officeonline/editor/" + docId, function(message) {
    ...
    var publishDocument = function(docId, data) {
      ...
      cometd.publish("/eXo/Application/Officeonline/editor/" + docId, data, cometdContext, function(publishReply) {
    ...
    var onError = function(event) {
      log("OFFICEONLINE Document Editor reports an error: " + JSON.stringify(event));
    };

    var onReady = function() {
      log("OFFICEONLINE Document Editor is ready");
    };

    var onBack = function() {
      log("OFFICEONLINE Document Editor on Back");
    };

    var onDocumentStateChange = function(event) {
      log("documentChange: " + JSON.stringify(event));
      if (event.data) {
        log("OFFICEONLINE The document changed");
        changesSaved = false;
        ...
        if (!changesSaved) {
          log("OFFICEONLINE Changes are collected on document editing service");
          changesSaved = true;
    ...
    var create = function(config) {
      ...
      if (config) {
        // prepare config
        log("OFFICEONLINE editor config: " + JSON.stringify(config));

        // customize Officeonline JS config
        config.type = "desktop";
        config.height = "100%";
        config.width = "100%";
        config.embedded = {
        ...
        config.editorConfig.customization = {
          ...
          "help" : true,
          "logo" : {
            "image" : prefixUrl + "/officeonline/images/exo-icone.png",
        ...
        // load Officeonline API script
        // XXX need load API script to DOM head, Officeonline needs a real element in <script> to detect the DS server URL
        ...
        function waitReady() {
          ...
          if (attempts >= 0) {
            ...
            log("ERROR: OFFICEONLINE script load timeout: " + config.documentserverJsUrl);
            process.reject("OFFICEONLINE script load timeout. Ensure Document Server is running and accessible.");
          }
        }
        ...
        if (localConfig) {
          ...
          $(function() {
            try {
              ...
              log("Error initializing Officeonline client UI " + e, e);
            }
          });
        } else {
  ...
  function UI() {
    ...
    var getNoPreviewEditorButton = function(editorLink) {
      return "<a class='btn editOnlineBtn hidden-tabletL' href='#' onclick='javascript:window.open(\"" + editorLink + "\");'>"
          + "<i class='uiIconEcmsOfficeonlineOpen uiIconEcmsLightGray uiIconEdit'></i>" + message("EditButtonTitle") + "</a>";
    };
    ...
    var saveAndDestroy = function() {
      if (docEditor) {
        ...
        try {
          ...
          log("Error saving and destroying OFFICEONLINE editor", e);
        }
      }
    };
    ...
    this.initEditor = function() {
      ...
      $("#SharedLayoutRightBody").addClass("officeonlineEditorBody");
    };

    this.isEditorLoaded = function() {
      return $("#UIPage .officeonlineContainer").length > 0;
    };
    ...
    this.showNotice = function(type, title, text, options) {
      var noticeOptions = {
        ...
        sticker : false,
        opacity : .9,
        addclass : 'officeonline-notification',
        shadow : true,
  ...
  $(function() {
    try {
      ...
      loadStyle("/officeonline/skin/jquery-ui.css");
      loadStyle("/officeonline/skin/jquery.pnotify.default.css");
      loadStyle("/officeonline/skin/jquery.pnotify.default.icons.css");
      loadStyle("/officeonline/skin/officeonline.css");
      ...
      log("Error configuring Officeonline Editor style.", e);
    }
  });
  return editor;
})($, cCometD, Redux);